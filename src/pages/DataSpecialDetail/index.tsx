...
export default memo(() => {
  ...
  const editSpecial = useCallback(
    async (params: Partial<API_INSTANCE.IPutInstanceSpecialParams>) => {
      const newValue = {
        ...detail,
        ...params,
  ...
  return (
    <PageContainer
      header={{
        ...
        extra: [
          ...
          <Button onClick={deleteSpecial} key="2" danger>
            删除
          </Button>,
          detail?.valid === false && (
            <Button
              type="primary"
              onClick={editSpecial.bind(null, { valid: true })}
              key="3"
            >
              启用
            </Button>
          ...
          detail?.valid === true && (
            <Button
              danger
              key="4"
              onClick={editSpecial.bind(null, { valid: false })}
            >
              禁用
            </Button>
          ),
    ...
    >
      {activeKey === 'base' && (
        <Descriptions loading={loading} onChange={editSpecial} value={detail} />
      )}
      {activeKey === 'info' && (
      ...
      <Form onSubmit={editSpecial} ref={modalRef} />
    </PageContainer>
  );
});
