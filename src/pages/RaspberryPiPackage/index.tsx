...
const RaspberryPiPackage = () => {
  ...
  const handleDelete = useCallback(async (record: API_RASPBERRY.GetListData) => {
    const { _id } = record;
  ...
  const handleReBuild = useCallback(async (record: API_RASPBERRY.GetListData) => {
    const { _id } = record;
  ...
  const onEditConfirm = useCallback(async (value: API_RASPBERRY.PutDataParams) => {
    const { _id } = value;
  ...
  const columns: any[] = useMemo(() => {
    return [
      ...column,
      {
        ...
        valueType: 'option',
        fixed: 'right',
        render: (_: any, record: API_RASPBERRY.GetListData) => {
          return (
            <Space>
              <Button
                style={{ padding: 0 }}
  ...
  }, [handleDelete, handleReBuild, handleEdit]);
