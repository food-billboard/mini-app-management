...
const ProportionClassify: React.FC<any> = ({
  ...
  const [classifyType, setClassifyType] = useState<keyof typeof EType>(
    EType.classify,
  );
  ...
  const chartData = useMemo(() => {
    ...
    const newList = newData.reduce<{ x: string; y: number; value: number }[]>(
      (acc, cur) => {
        ...
        <div className={styles.salesCardExtra}>
          <div className={styles.salesTypeRadio}>
            <Radio.Group value={classifyType} onChange={onClassifyChange}>
              <Radio.Button value={EType.classify}>分类</Radio.Button>
              <Radio.Button value={EType.movie}>来源类型</Radio.Button>
            </Radio.Group>
          </div>
        </div>
      }
