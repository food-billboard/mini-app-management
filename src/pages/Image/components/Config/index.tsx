...
const Config = forwardRef<IImageConfigRef, IProps>((props, ref) => {
  ...
  const rotate: (val: string | number | undefined) => ViewerInstance = useCallback(
    (val) => {
  ...
  const rotateTo: (val: string | number | undefined) => ViewerInstance = useCallback(
    (val) => {
      let value = typeof val === 'number' ? val : parseFloat(val ?? '0');
      value = Number.isNaN(value) ? 0 : value;
  ...
  return (
    <Drawer open={visible} title={'图片操作'} {...props} onClose={close}>
      {!props.instance && (
        <Result status="404" title="404" subTitle="对不起，未知原因导致无法启动图片预览功能" />
      )}
      {!!props.instance && (
        <Descriptions
          bordered
        ...
        >
          <DescriptionItem label="移动">
            ...
            >
              <Button type="primary" onClick={moveTop}>
                上
              </Button>
              <Button type="primary" onClick={moveBottom}>
                下
              </Button>
              <Button type="primary" onClick={moveLeft}>
                左
              </Button>
              <Button type="primary" onClick={moveRight}>
                右
              </Button>
            </div>
          </DescriptionItem>
          <DescriptionItem label="旋转">
            <InputNumber defaultValue={90} onChange={rotate as any} style={{ width: '100%' }} />
          </DescriptionItem>
          <DescriptionItem label="旋转至">
            <InputNumber defaultValue={270} onChange={rotateTo as any} style={{ width: '100%' }} />
          </DescriptionItem>
        </Descriptions>
      )}
    </Drawer>
